...
author: "Lab 02 by Md Forhad Akbar"
date: '2019-09-08'
output:
  pdf_document:
    toc: yes
...
Number of cases:    
    ```{r number-of-cases, eval=TRUE}
nrow(cdc)
```
Number of variables:
    ```{r number-of-variables, eval=TRUE}
length(cdc)
```
Number of cases and variables:
    ```{r number-of-cases-and-variables, eval=TRUE}
dim(cdc)
```
Data type:
...
We can just run nrow and lenth commad which will give us total number of rows and columns in a given data frame. We can also run dim command which will give us both total number of rows and variables or columns. We can see there are 20000 cases or rows and 9 variables.

Data type of each variables:
1. genhlth: categorical, ordinal, qualitative
2. exerany: categorical, nominal, qualitative
3. hlthplan: categorical, nominal, qualitative
4. smoke100: categorical, nominal, qualitative
5. height: numerical, discrete (as it is countable here), quantitative
6. weight: numerical, discrete (as it is countable here), quantitative
7. wtdesire: numerical, discrete (as it is countable here), quantitative
8. age: numerical, discrete (as it is countable here), quantitative
9. gender: categorical, nominal, qualitative
...
Interquartile range (IQR) for `height`: using IQR command instead of manually subtract 1st Qu. from 3rd Qu.
...
Interquartile range (IQR) for `age`: using IQR command instead of manually subtract 1st Qu. from 3rd Qu.
```{r iqr-age, eval=TRUE}
IQR(cdc$age)
```    
    
Relative frequency distribution for `gender`: using nrow command instead manually dividing by 20000
```{r rfd-gender, eval=TRUE}
rfdgender<- table(cdc$gender)/ nrow(cdc)
rfdgender
```
    
Relative frequency distribution for `exerany`: using nrow command instead manually dividing by 20000
```{r rfd-exerany, eval=TRUE}
rfdexerany<- table(cdc$exerany)/ nrow(cdc)
rfdexerany
```   
    
How many males are in the sample: 
```{r table-gender, eval=TRUE}
tablegender<- table(cdc$gender)
tablegender
```   
There are 9569 males

What proportion of the sample reports being in excellent health:    
```{r table-health, eval=TRUE}
rfdgenhlth<- table(cdc$genhlth)/nrow(cdc)
rfdgenhlth
```    
```{r table-health-percent, eval=TRUE}
percent(rfdgenhlth, 3)
```
23.29% report being in excellent health

### Extra
Create a mosaic plot to see gender and health:

```{r table-gender-health, eval=TRUE}
genderhealth<- table(cdc$gender,cdc$genhlth)
genderhealth
```
```{r mosaic-gender-health, eval=TRUE}
mosaicplot(genderhealth, main= "General Health by gender", xlab = "Gender", ylab = "General health")
```
...
table(cdc$gender,cdc$smoke100)
```
Answer: It looks like males seems to be more likely to smoke than females but the difference is not very drastic. The mosaic plot reveals that a randomly selected male is more probable to be a person who smoked at least 100 cigarettes than a randomly selected female to be a person who smoked at least 100 cigarettes.
...
Answer: Based on above plot it looks like higher BMI seems to correlate with poor health.

```{r boxplot-bmi-exercise, eval=TRUE}
boxplot(bmi ~ cdc$exerany, main= "BMI by exercise", xlab = "Exercise", ylab = "BMI")
```
Answer: The variable I chose is Exercise, I want to see if exercising correlates with lower BMI. I thought the people do exercise should have ideal BMI. The plot seems to suggest a slight correlation. but is not as significant as I expected.
...
Answer:
The histogram with fewer bins  shows the mean to be in the 20-25 bin the histogram with 50 bins shows sharp peak in the 25 bin, right at the cutoff from normal to over weight, with a dip in the data after that. I read how R chooses the breaks by default. It was surprising to see it has some C implementation. I think it is alwasys better to choose a good breaks in order to communicate histrograms well. 
...
Using plot function:   
```{r plot-weight-wtdesire, eval=TRUE}
plot(x= cdc$weight, y= cdc$wtdesire, main = "Scatterplot of weight versus desired weight",
     xlab = "Weight", ylab = "Desired weight",
     pch = 19, frame = FALSE)
abline(lm(cdc$wtdesire ~ cdc$weight, data = cdc), col = "blue")
```    

Using scatterplot function from car package:
```{r scatterplot-weight-wtdesire, eval=TRUE}
scatterplot(cdc$wtdesire~cdc$weight, data = cdc)
```  

Answer: There seems to be a positive correlation between weight and desired weight, this does really tell me much since a 250 pound person may want to be 225 and a 175 pound person may want to be 165 so the desired weight is going to be in close proimity to the actually weight making them positively correlated.
...
```{r wdiff-head, eval=TRUE}
head(wdiff)
```

```{r wdiff-tail, eval=TRUE}
tail(wdiff)
```

```{r wdiff-summ, eval=TRUE}
summary(wdiff)
``` 

```{r wdiff-class, eval=TRUE}
class(wdiff)
```

Answer: wdiff is numerical, discrete, quantitative  data, a negative value means that the person wants to loss weight since wtdesire is first in the subtraction. wdiff is 0 means the person happy with the weight and wants to stay the same weight. A positive value means that the person wants to gain weight.
...
Check summary and IQR to confirm boxplot 
```{r wdiff-summary, eval=TRUE}
summary(wdiff)
```

```{r wdiff-IQR, eval=TRUE}
IQR(wdiff)
```

```{r wdiff-boxplot, eval=TRUE}
boxplot(wdiff)
```

```{r wdiff-hist, eval=TRUE}
hist(wdiff,breaks=100)
```
The data is left skewed with a center just less than 0, meaning that most people want to loss weight. The 3rd quartile is 0 and the IQR is 21 this means that 75% of people want to loss between 0 and 21 pounds.  The box plot shows there are many the will like to loose more weight than average but not enough to make it left skewed by a lot.
...
Answer: Women (median = -10) generally appear to want to lose a few more pounds than men (median = -5), and women have a slightly larger range of how much they want to lose/gain (iqr = 27) than men (iqr = 20). Interestingly, more men than women appear to want to gain weight.
...
```{r 1-sd- weight}
instdev <- subset(cdc, weight < (meanwt + sdwt) & weight > (meanwt - sdwt))
dim(instdev)[1]/dim(cdc)[1]
```
Answer: 
mean of weight = 169.7
standard deviation of weight = 40.08
proportion within one standard deviation of the mean = .7076 or 70%
