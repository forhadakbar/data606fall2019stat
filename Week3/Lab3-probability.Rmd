...
Answer: A streak length of 1 mean a hit followed by a miss. A streak length of 0 mean a miss followed by miss.
...
Answer: The typical streak length is 0. The longest streak was 4. The distribution is right skewed and unimodal with a range of 0 to 4, a mode of 0 and a median of 0.
...
```{r sim-basket-change, eval=TRUE}
outcomes <- c("H", "M")
sim_basket <- sample(outcomes, size = 133, replace=TRUE, prob = c(0.45,0.55))
sim_basket
```

```{r sim-basket-table, eval=TRUE}
table(sim_basket)
...
```{r streak-lengths-sim_basket, eval=TRUE}
...
The distribution of streak lengths is unimodal and right skewed. Typical streak length is 1. The longest streak is 5-8. Note here, that longest streak will be different everytime we run the code as it is a sample simulation.
...
Answer: I would expect it to be somewhat similar to the simulation above with the same probability of 0.45. Since they are independent and we are looking at the same amount of shots (133), i would expect the result to be similar.  
...
Answer: Both distribution are fairly similar to each other. Both are unimodal and right skewed. However, the simulated shooter has better longest streak (8), compare to Kobe’s longest streatk length. I don't see any evidence that the hot hand model fits Kobe’s shooting pattern. The event of shots are independent from each other.
